class LocalAuthority < ActiveRecord::Base
  ...
  def self.harvest_rdf_sparql(name, sources, sparql, opts: {}, append: false)
    # if append==false and authority already exists, don't do anything
    authority = self.find_by(name: name)
    return if (not append) and authority
    # create authority if it doesn't exist
    authority||= self.create(name: name)

    # Initialise sparql query and rdf repository. Accept preparsed queries
    # or already loaded repositories, otherwise parse or load them here.
    if sparql.is_a? SPARQL::Algebra::Operator
      query=sparql
    else
      query=SPARQL.parse(sparql)
    end
    if sources.is_a? RDF::Repository
      repo=sources
    else
      repo = RDF::Repository.load(sources, opts)
    end

    entries = []

    # execute query and build authority entries
    query.execute(repo) do |res|
      res=yield res if block_given?
      if res
        hasAlt = ( res[:altLabel] and not res[:altLabel].to_s.empty? )
        entries << LocalAuthorityEntry.new(local_authority: authority,
                                           label: (hasAlt ? res[:altLabel].to_s : res[:label].to_s ),
                                           uri: res[:term].to_s,
                                           use_label: (hasAlt ? res[:label].to_s : nil )
                                           )
      end
    end
  ...
  def self.entries_by_term(model, term, query)
    ...
    if (term == 'subject' && model == 'generic_files') # and local_authoritiy = lc_subject
      ...
      if dterm
        ...
        sql = LocalAuthorityEntry.where("local_authority_id in (?)", authorities).where("lower(label) like ?", "#{lowQuery}%").select("label, uri, use_label").limit(25).to_sql
